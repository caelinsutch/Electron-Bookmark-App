...
exports.changeItem = (direction) => {
    ...
    // Only if new item exists, make selection change
    if (newItem.length) {
...
// Delete item
window.deleteItem = (i) => {
    // Set i to active item if not passed
    if (!i) {
        i = $('.read-item.isActive').index() - 1;
    ...
    if (this.toreadItems.length) {
        ...
        // If first item was deleted, select new first item
        let newIndex = (i === 0) ? 0 : i-1

        // Asign active class to new index
...
window.openItem = () => {
    ...
    // Get index item to pass to proxy window
